...
/// Row of four large, clickable overview cards (All / Healthy / Low /
/// Out of stock). Doubles as the desktop filter bar — selecting a card
/// drives [InventoryBloc]'s [StockFilter] the same way the mobile mini
/// stats do, but with room for a value, a trend bar and a hover state.
class DesktopInventoryStatsRow extends StatelessWidget {
  final List<StockData> stockList;
  final StockFilter selectedFilter;
  final ValueChanged<StockFilter> onFilterChanged;

  const DesktopInventoryStatsRow({
    super.key,
  ...
  Widget build(BuildContext context) {
    ...
    final outOfStock = stockList.where((e) => e.isOutOfStock == true).length;
    final lowStock = stockList
        .where((e) => e.isLowStock == true && e.isOutOfStock != true)
        .length;
    final healthy = total - outOfStock - lowStock;
    ...
    final cards = [
      ...
      _StatCardData(
        ...
        value: outOfStock,
        total: total,
        icon: SolarIconsOutline.cartCross,
        color: colors.danger,
      ),
    ];
